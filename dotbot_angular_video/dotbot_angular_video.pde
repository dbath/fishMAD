...
Dots group1 = new Dots(3000,0,1,100, true);
...
void setup(){
  ...
  size(2000,1600);//fullScreen();
  ellipseMode(CENTER);
  sqBar = 175;//(width - height) /2;
...
void draw(){   
    ...
    if  ( myFrameCount >= 150){
        if (currentEpoch == 0 ){ hideMode = 0;
                                      group1.Set(int(nDots*(C)), 0, -1);
                                      group2.Set(int(nDots*(1-C)), 0, -1);
                                      }
        else if (currentEpoch == 1 ){ hideMode = 0;
                                      group1.Set(int(nDots*(C)), speed, -1);
                                      group2.Set(int(nDots*(1-C)), speed, -1);
                                      }
        else if (currentEpoch == 18 ){ hideMode = 0;
                                      group1.startCoherent();
                                      //group1.Set(int(nDots*(C)), speed, -1);
                                      //group2.Set(int(nDots*(1-C)), speed, 1);
                                      }
        else if (currentEpoch == 36 ){ hideMode = 0;
                                      group1.startRandom();
                                      //group1.Set(int(nDots*(C)), 0, 1);
                                      //group2.Set(int(nDots*(1-C)), 0, 1);
                                      }
        else if (currentEpoch == 54 ){ hideMode = 1;
                                      group1.startRandom();
                                      //group1.Set(int(nDots*(C)), 0, 1);
                                      //group2.Set(int(nDots*(1-C)), 0, 1);
                                      }
