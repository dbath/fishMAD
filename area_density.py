...
if __name__ == "__main__":
    ...
    if 0:#os.path.exists('/media/recnodes/Dan_storage/' + args.handle + '_prestim_area_measures.pickle'):
        results = pd.read_pickle('/media/recnodes/Dan_storage/' + args.handle + '_prestim_area_measures.pickle')
    else:
    ...
    for MAIN_DIR in glob.glob(args.dir + '/'+ args.handle + '*.stitched'):
        ...
        try:        
            ...
            prestim = synced[synced.syncTime.between(np.timedelta64(-30, 's'), np.timedelta64(0,'s'))]
            ...
            for group, data in prestim.groupby('trialID'):
                ...
                results.to_pickle('/media/recnodes/Dan_storage/' + args.handle + '_prestim_area_measures.pickle')
                print(s)
    ...
    #                                            left_index=True,
    #                                            right_index=True) 
    results.to_pickle('/media/recnodes/Dan_storage/' + args.handle + '_prestim_area_measures_complete.pickle')                          
