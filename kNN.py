...
def plot_hist(fig, ax, dataArray, nbins, xLabel, xLim):
    ...
    ax.set_xlabel(xLabel, fontsize=12, labelpad=0.2)
    ax.set_xlim(xTicks[0], xTicks[-1])
    ax.set_xticks(xTicks)
    ax.tick_params(axis='x', which='major', labelsize=12, length=1, width=0.1)
    ax.tick_params(axis='y', which='major', labelsize=12, length=0, width=0.0)
    """
    ax.set_ylim(0,0.05)  
    ax.set_yticks([])
