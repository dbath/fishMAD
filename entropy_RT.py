...
def getOnlyReversals(df, _THRESHOLD=-0.1):
    ...
    for ID, h in g:
        ...
        if pre['median_dRotation_cArea'].mean() > _THRESHOLD: #remove trials where response is ambiguous
            continue
        h['trialID'] = ID
...
def set_threshold(gd, THRESHOLD, COL):
    #FIXME i changed correct/incorrect for entropy cuz its inverted. lazy danno
    gd.loc[gd[COL] <= 0.95*THRESHOLD, 'correct'] = 1
    gd.loc[gd[COL] > 1.05*THRESHOLD, 'correct'] = 0
    gd.loc[gd[COL] > 1.05*THRESHOLD, 'incorrect'] = 1
    gd.loc[gd[COL] <= 0.95*THRESHOLD, 'incorrect'] = 0
...
def get_kinetics(groupData, col='entropy_normed'):
    ...
    for trial, data in g:
        data = data.copy()
        prestim_period = data.loc[data['syncTime'] < np.timedelta64(0)]
        
        #DISCARD TRIALS WHERE RESPONSE IS AMBIGUOUS
        if prestim_period['median_dRotation_cArea'].mean() > PRESTIM_THRESHOLD:
            continue 
        ...
        THRESHOLD = prestim_period[col].mean()
        ...
        try:
            if stim_period['correct'].mean() > 0.65: #FIXME so much thresholding
    ...
    bouts.to_pickle('/media/recnodes/Dan_storage/191121_bouts_of_entropy.pickle')
    ...
    plotnice()
    plt.savefig('/media/recnodes/Dan_storage/191121_proportion_correct_trials_vs_groupsize_for_entropy.svg')
    
    for col in ['IBI','duration', 'RT', 'DecayTime']:
        plt.close('all')
        ...
        plt.xlabel('Stimulus coherence')
        plotnice()
        plt.savefig('/media/recnodes/Dan_storage/191121_' + col + '_vs_groupsize.svg')
...
PRESTIM_THRESHOLD = 0.1

bouts = get_kinetics(allData)
