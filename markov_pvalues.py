...
SAVEDIR = '/home/dbath/Documents/170322_claire_vcode_data/170928_repeat_with_following'
scores = ['following','wing extension','medPE','attempted copulation','bothlegs']
behaviours = ['a-F','b-W','c-P','d-A', 'e-L']
...
t, c, d = permute_CI_values(model, observed_states, lengths, 100)
...
pd.DataFrame(t, columns = idt, index=idt).to_csv(SAVEDIR + '/' + fn_string + fn_string2 + 'transition_matrix.csv')
pd.DataFrame(c, columns = idt, index=idt).to_csv(SAVEDIR + '/' + fn_string + fn_string2 + 'CI_t_less_than_random.csv')
pd.DataFrame(d, columns = idt, index=idt).to_csv(SAVEDIR + '/' + fn_string + fn_string2 + 'CI_t_more_than_random.csv')
